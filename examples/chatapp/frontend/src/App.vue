...
// 当前视图状态：'landing' (展示页) 或 'chat' (聊天页)
const currentView = ref<'landing' | 'chat'>('landing')
const selectedMode = ref<'extended' | 'compact' | 'floating'>('floating')
  ...
  <div v-if="currentView === 'landing'" class="landing-page">
    <div class="container">
      ...
      <div class="section">
        ...
        <h3>安装</h3>
        <pre>npm install ai-chatbot</pre>
...
app.mount('#app')</pre>
      ...
      <!-- Footer -->
      <footer>
        <p>AI Chatbot Component &copy; 2024</p>
      </footer>
    </div>
  </div>
  ...
  <div v-else class="chat-page">
    <!-- 返回按钮 (仅在扩展模式显示) -->
    <button v-if="selectedMode === 'extended'" class="back-button" @click="backToLanding">
      ← 返回首页
    </button>
...
.mode-card.primary h2,
.mode-card.primary .subtitle,
.mode-card.primary .description {
  color: white;
}
