...
| SidebarDemo | Demo演示页 | /sidebar | 边栏模式演示页面 |
...
| **边栏模式** | `/sidebar` | SaaS后台、客服平台 | Tab切换会话/聊天视图，桌面端侧边栏固定 |
| **紧凑模式** | `/compact` | 内嵌页面 | 桌面端侧边栏固定，移动端自动全屏 |
...
```typescript
const config = {
  mode: 'floating',  // iframe内部使用floating模式
  iframeMode: true,
  // 通过postMessage通信
}
```
